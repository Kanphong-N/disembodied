...
public class Minion {
    ...
    // config
    private final float frameDuration = MathUtils.randomWithin(0.15f, 0.25f); // how fast the animation should be
    private final Array<Key> keySequence;
    ...
    private float walkingSpeed = MathUtils.randomWithin(0.1f, 0.2f);
    private float jumpingSpeed = MathUtils.randomWithin(2f, 3f);
    ...
    public void render(MyShapeRenderer sr) {
        ...
        if (hasKeysLeftToVisit()) {
            ...
            sr.line(getCenterX(), getCenterY(), nextKey.getCenterX(), nextKey.getCenterY(), Color.WHITE, Color.DARK_GRAY);
        }
    }
